...
Command summary: I first used command ls to see what was inside the current directory. 
Then I used command cd to go into the folder "lab7" I cloned. 
Next, I used command ls to see what was inside the "lab7" folder. 
I saw the "test.sh" was here so I type "bash tes". 
I knew that was the only file that started with "tes", so I used \<tab\> to come up with the full name and ran the command bash test.sh to run the test.
